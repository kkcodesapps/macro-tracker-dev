...
const MealList = () => {
  ...
  const toggleMealExpansion = async (mealId: number) => {
    if (expandedMeals.includes(mealId)) {
      ...
      try {
        ...
        setLocalMeals(prevMeals => prevMeals.map(meal => {
          if (meal.id === mealId) {
            return {
              ...meal,
  ...
  const handleDragStart = (e: React.MouseEvent | React.TouchEvent, mealId: number) => {
    e.stopPropagation();
  ...
  const handleDragMove = (e: React.MouseEvent | React.TouchEvent, mealId: number) => {
  ...
  const handleMealClick = (e: React.MouseEvent, mealId: number) => {
    ...
    if (!(e.target as HTMLElement).closest('.meal-action-button') && 
        swipedMealId !== mealId && 
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4">
          <button onClick={goToPreviousDay} className={`p-1 rounded-full ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}>
            <ChevronLeft className="h-6 w-6" />
          </button>
          <span className="text-lg font-medium">{format(selectedDate, 'MMMM d, yyyy')}</span>
          <button onClick={goToNextDay} className={`p-1 rounded-full ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}>
        ...
        <div className={`overflow-hidden`}>
          <ul className="space-y-4">
            {localMeals.map((meal) => (
              <li 
                key={meal.id}
                className={`shadow px-6 py-4 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} relative transition-all duration-200 ease-in-out cursor-pointer ${swipedMealId === meal.id && 'rounded-r-none'}`}
                style={{
                  transform: swipedMealId === meal.id ? 'translateX(-60px)' : 'translateX(0)',
                }}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className={`text-lg font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>{meal.name}</h3>
                  <div className="flex items-center">
                    <button
                      ...
                      className={`meal-action-button mr-2 p-1 rounded-full ${darkMode ? 'hover:bg-gray-600' : 'hover:bg-gray-200'}`}
                    >
                ...
                <div className="grid grid-cols-4 gap-4 mb-2">
                  <div className={`p-2 rounded ${darkMode ? 'bg-blue-900' : 'bg-blue-100'}`}>
                    <p className={`text-sm font-medium ${darkMode ? 'text-blue-200' : 'text-blue-800'}`}>Calories</p>
                    <p className={`text-lg font-bold ${darkMode ? 'text-blue-100' : 'text-blue-900'}`}>{meal.calories}</p>
                  </div>
                  <div className={`p-2 rounded ${darkMode ? 'bg-red-900' : 'bg-red-100'}`}>
                    <p className={`text-sm font-medium ${darkMode ? 'text-red-200' : 'text-red-800'}`}>Protein</p>
                    <p className={`text-lg font-bold ${darkMode ? 'text-red-100' : 'text-red-900'}`}>{meal.protein}g</p>
                  </div>
                  <div className={`p-2 rounded ${darkMode ? 'bg-green-900' : 'bg-green-100'}`}>
                    <p className={`text-sm font-medium ${darkMode ? 'text-green-200' : 'text-green-800'}`}>Carbs</p>
                    <p className={`text-lg font-bold ${darkMode ? 'text-green-100' : 'text-green-900'}`}>{meal.carbs}g</p>
                  </div>
                  <div className={`p-2 rounded ${darkMode ? 'bg-yellow-900' : 'bg-yellow-100'}`}>
                    <p className={`text-sm font-medium ${darkMode ? 'text-yellow-200' : 'text-yellow-800'}`}>Fat</p>
                    <p className={`text-lg font-bold ${darkMode ? 'text-yellow-100' : 'text-yellow-900'}`}>{meal.fat}g</p>
                  </div>
                </div>
                {expandedMeals.includes(meal.id) && meal.foods && (
                  <div className="mt-4">
                    <h4 className={`text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Foods in this meal:</h4>
                    <ul className="space-y-2">
                      {meal.foods.map((food) => (
                        <li key={food.id} className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                          ...
                          {food.is_quick_macro && <span className="ml-2 text-xs italic">(Quick Macro)</span>}
                        </li>
                      ))}
                    </ul>
                  </div>
